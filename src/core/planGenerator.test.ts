...
describe('planGenerator (Повний Інтеграційний Тест)', () => {
  ...
  describe('Перевірка розрахунків для Bench Press (Linear)', () => {
    ...
    it('Тиждень 1', () => {
      expect(
        plan.weeklyBreakdown[0]!.exercises[exerciseIndex]!.workingSets.weightKg
      ).toBeCloseTo(80);
    });
    it('Тиждень 2', () => {
      expect(
        plan.weeklyBreakdown[1]!.exercises[exerciseIndex]!.workingSets.weightKg
      ).toBeCloseTo(82.5);
    });
    it('Тиждень 3', () => {
      expect(
        plan.weeklyBreakdown[2]!.exercises[exerciseIndex]!.workingSets.weightKg
      ).toBeCloseTo(85);
    });
    it('Тиждень 4', () => {
      expect(
        plan.weeklyBreakdown[3]!.exercises[exerciseIndex]!.workingSets.weightKg
      ).toBeCloseTo(85);
    });
  });
  ...
  describe('Граничні випадки (Edge Cases)', () => {
    ...
    it('повинен розраховувати ваги як 0, якщо startCoefficient: 0', () => {
      ...
      expect(
        plan.weeklyBreakdown[0]!.exercises[0]!.workingSets.weightKg
      ).toBeCloseTo(0);
    });
  });
});

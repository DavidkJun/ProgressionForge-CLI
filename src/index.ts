  ...
  switch (command) {
    ...
    case 'generate': {
      ...
      try {
        ...
        recipe = loadedData as PlanRecipe;
      } catch (error) {
        console.error(
          error instanceof Error ? error.message : 'Failed to load plan recipe.'
        );
        process.exit(1);
      }
      ...
      if (!validationResult.success) {
        console.error(
          `Plan file "${fileName}" is corrupted or has invalid format:`
        );
    ...
    case 'delete': {
      ...
      try {
        ...
        console.error(
          `Failed to delete plan "${fileName}":`,
          err instanceof Error ? err.message : err
        );
        process.exit(1);
      }
    ...
    default:
      ...
      console.error(
        '  generate <plan-name> - Generates a workout schedule from a saved plan.'
      );

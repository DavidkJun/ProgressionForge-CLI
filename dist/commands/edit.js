...
import { validateFloat, validateNonEmptyString, validatePositiveNumber, validatePositiveInteger, } from '../utils/validators.js';
const editSingleExercise = async (exercise) => {
    let editing = true;
...
const manageExercises = async (recipe) => {
    let managing = true;
    while (managing) {
        const choices = [
            'Add a new exercise',
        ...
        if (action === 'Add a new exercise') {
            ...
            if (index !== -1) {
                recipe.exercises[index] = await editSingleExercise(recipe.exercises[index]);
            }
        }
...
export const handleEdit = async (planName) => {
    try {
        ...
        const recipe = (await loadPlan(planName));

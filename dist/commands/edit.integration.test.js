...
describe('Command: edit (Integration)', () => {
    ...
    it('should show error if plan does not exist', async () => {
        ...
        storageManager.planExists.mockResolvedValue(false);
        await handleEdit(planName);
        expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining(`Error: Plan "${planName}" does not exist`));
        expect(storageManager.loadPlan).not.toHaveBeenCalled();
    });
    it('should update plan duration and bodyweight', async () => {
        storageManager.planExists.mockResolvedValue(true);
        storageManager.loadPlan.mockResolvedValue({ ...mockPlan });
        inquirer.prompt
        ...
        expect(storageManager.savePlan).toHaveBeenCalledWith('TestPlan', expect.objectContaining({
            durationWeeks: 8,
            bodyWeightKg: 85,
        }));
    });
    it('should edit an existing exercise (Sets, Reps, Coefficient)', async () => {
        storageManager.planExists.mockResolvedValue(true);
        storageManager.loadPlan.mockResolvedValue(JSON.parse(JSON.stringify(mockPlan)));
        inquirer.prompt
            .mockResolvedValueOnce({
    ...
    it('should add a new exercise using newCommand.promptForExercise', async () => {
        ...
        newCommand.promptForExercise.mockResolvedValue(newExerciseMock);
        inquirer.prompt
            .mockResolvedValueOnce({
            mainAction: 'Manage Exercises',
        })
...
//# sourceMappingURL=edit.integration.test.js.map
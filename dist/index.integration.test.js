...
describe('CLI Integration Test (index.ts)', () => {
    ...
    describe('Command: new', () => {
        ...
        it('should fail if filename is missing', async () => {
            ...
            expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Missing plan name'));
    ...
    describe('Command: delete', () => {
        ...
        it('should handle errors during deletion', async () => {
            ...
            storageManager.deletePlan.mockRejectedValue(new Error(errorMsg));
            await runCli(['delete', 'OldPlan']);
            expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('Failed to delete plan'), expect.stringContaining(errorMsg));
            expect(mockExit).toHaveBeenCalledWith(1);
        });
    });
    describe('Command: list', () => {
        it('should list plans if they exist', async () => {
            storageManager.listPlans.mockResolvedValue([
                'plan1.json',
                'plan2.json',
            ]);
            await runCli(['list']);
            expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Available plans'));
            expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('plan1.json'));
        });
    ...
    describe('Command: generate', () => {
        ...
        it('should fail if loadPlan throws error', async () => {
            storageManager.loadPlan.mockRejectedValue(new Error('Read error'));
        ...
        it('should fail if validation fails (Zod)', async () => {
            storageManager.loadPlan.mockResolvedValue({
                invalid: 'data',
            });
            recipePlanSchema.safeParse.mockReturnValue({
                success: false,
                error: { format: () => ({ _errors: ['Invalid data'] }) },
            });
            await runCli(['generate', 'TestPlan']);
            expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('corrupted or has invalid format'));
            expect(mockExit).toHaveBeenCalledWith(1);
        });
        it('should generate and print plan if data is valid', async () => {
            storageManager.loadPlan.mockResolvedValue(validPlanData);
            recipePlanSchema.safeParse.mockReturnValue({
                success: true,
                data: validPlanData,
            });
            const mockGeneratedPlan = { week1: 'workout' };
            planGenerator.generatePlan.mockReturnValue(mockGeneratedPlan);
            await runCli(['generate', 'TestPlan']);
    ...
    describe('Default (Unknown command)', () => {
        it('should show help and exit', async () => {
            await runCli(['dance']);
            expect(mockConsoleError).toHaveBeenCalledWith('Unknown or missing command.');
            expect(mockExit).toHaveBeenCalledWith(1);
        });
    });
});

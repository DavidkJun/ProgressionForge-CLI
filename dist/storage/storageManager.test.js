...
describe('storageManager (Integration Tests)', () => {
    ...
    describe('savePlan & loadPlan', () => {
        ...
        it('should throw an error if the plan does not exist', async () => {
            await expect(storageManager.loadPlan('non-existent')).rejects.toThrow('does not exist');
        });
        it('should throw an error if the file is corrupted (invalid JSON)', async () => {
            ...
            await expect(storageManager.loadPlan('bad-json')).rejects.toThrow('Failed to load or parse plan');
        });
        it('should throw a validation error if the data structure is invalid', async () => {
            ...
            await expect(storageManager.loadPlan('invalid-schema')).rejects.toThrow('is corrupted or has invalid format');
        });
    });
    describe('listPlans', () => {
    ...
    describe('deletePlan', () => {
        ...
        it('should throw an error when trying to delete a non-existent plan', async () => {
            await expect(storageManager.deletePlan('Ghost')).rejects.toThrow('does not exist');
        });
    });
    describe('planExists', () => {
